...
plt.plot(range(1, 11), results, marker='o', linestyle='-', color='purple', label='Accuracy từng lần')
plt.axhline(avg_acc, color='red', linestyle='--', label=f'Accuracy trung bình: {avg_acc:.4f}')
plt.title('Biểu đồ độ chính xác của Naive Bayes qua 10 lần chạy')
plt.xlabel('Lần chạy')
plt.ylabel('Accuracy')
...
print("💾 Đã lưu model vào 'naive_bayes_model.pkl'")
